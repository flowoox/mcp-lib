...
from .config import FailoverClusterTarget
...
class PowerShellExecutionError(RuntimeError):
    """A static Failover Cluster diagnostic probe failed or returned invalid bounded JSON."""


class PowerShellRunner:
    """Run repository-owned read-only Failover Cluster probes locally or through constrained JEA."""

    def __init__(self, executable: str):
        executable = executable.strip()
        if not executable:
            raise ValueError("FAILOVERCLUSTER_POWERSHELL_EXECUTABLE must not be blank")
        basename = executable.replace("\\", "/").rsplit("/", 1)[-1].casefold()
        if basename not in _ALLOWED_EXECUTABLES:
            raise ValueError("FAILOVERCLUSTER_POWERSHELL_EXECUTABLE must resolve to PowerShell or pwsh")
...
if ($output.Count -ne 1) {{ throw 'Remote Failover Cluster probe returned an unexpected output shape' }}
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            raise ValueError(f"Unknown Failover Cluster script id: {script_id}") from exc
        ...
        if len(payload_json.encode("utf-8")) > _MAX_INPUT_BYTES:
            raise ValueError("Failover Cluster diagnostic payload exceeds the fixed input byte limit")
            ...
            with tempfile.TemporaryFile(mode="w+b") as stdout_file, tempfile.TemporaryFile(
                ...
                if stdout_size > max_response_bytes:
                    raise PowerShellExecutionError(
                        "Failover Cluster diagnostic probe exceeded the configured response byte limit"
                    )
                stdout_file.seek(0)
                raw = stdout_file.read(max_response_bytes + 1)
                if completed.returncode != 0:
                    stderr_file.seek(max(0, stderr_size - _MAX_ERROR_DETAIL_BYTES))
                    detail = stderr_file.read(_MAX_ERROR_DETAIL_BYTES).decode("utf-8", errors="replace")
                    detail = detail.replace("\x00", "").strip()
                    suffix = f": {detail}" if detail else ""
                    raise PowerShellExecutionError(
                        f"Failover Cluster probe {script_id.value} failed with exit code {completed.returncode}{suffix}"
                    )
        except subprocess.TimeoutExpired as exc:
            raise PowerShellExecutionError(
                f"Failover Cluster probe {script_id.value} timed out after {timeout_seconds:.1f}s"
            ) from exc
        ...
        if not raw:
            raise PowerShellExecutionError(
                f"Failover Cluster probe {script_id.value} returned no JSON output"
            )
        try:
            ...
            raise PowerShellExecutionError(
                f"Failover Cluster probe {script_id.value} returned invalid UTF-8 JSON"
            ) from exc
        if not isinstance(result, dict):
            raise PowerShellExecutionError(
                f"Failover Cluster probe {script_id.value} must return a JSON object"
            )

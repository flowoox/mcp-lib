...
def _identity(actor: str, source: str) -> tuple[str, str]:
    ...
    return normalized_actor, normalized_source
...
async def _observe(
    *,
    ...
    normalized_actor, normalized_source = _identity(actor, source)
    base_event = {
        "operation": operation,
        "phase": "observe",
        "risk": "read_only",
...
def build_server(settings: Settings) -> FastMCP:
    ...
    async def ad_observe_domain_policy(
        ...
        """Read the effective domain-root password and lockout policy attributes."""
    ...
    async def ad_find_user(
        ...
        The service constructs and escapes the LDAP filter; raw filters and
        arbitrary attributes are intentionally unavailable.
        """
    ...
    async def ad_get_group_members(
        ...
        if limit < 1 or limit > settings.ad_max_results:

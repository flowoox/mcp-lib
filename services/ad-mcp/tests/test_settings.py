...
def test_secret_values_are_only_exposed_through_explicit_accessors() -> None:
    settings = make_settings(mcp_auth_token=SecretStr("runtime-token"))
    assert "runtime-token" not in repr(settings)
    assert settings.auth_token == "runtime-token"

...
_PROJECTIONS: dict[FortiGateEndpoint, dict[str, tuple[str, ...]]] = {
    FortiGateEndpoint.SYSTEM_STATUS: {
        ...
        "serial": ("serial", "serial_number", "serial-number"),
        "version": ("version",),
        "build": ("build",),
    ...
    FortiGateEndpoint.HA_CONFIGURATION: {
        ...
        "priority": ("priority",),
        "override": ("override",),
        "sessionPickup": ("session-pickup", "session_pickup"),
        "heartbeatDevices": ("hbdev",),
        "monitoredInterfaces": ("monitor",),
    ...
    FortiGateEndpoint.INTERFACES: {
        ...
        "status": ("status",),
        "role": ("role",),
        "allowAccess": ("allowaccess", "allow-access"),
        "vlanId": ("vlanid", "vlan-id"),
    ...
    FortiGateEndpoint.STATIC_ROUTES: {
        "sequence": ("seq-num", "seq_num", "_mkey"),
        "destination": ("dst",),
        "gateway": ("gateway",),
        "device": ("device",),
        "distance": ("distance",),
    ...
    FortiGateEndpoint.FIREWALL_POLICIES: {
        "policyId": ("policyid", "policy-id", "_mkey"),
        "uuid": ("uuid",),
        "name": ("name",),
        "status": ("status",),
        "sourceInterfaces": ("srcintf",),
        "destinationInterfaces": ("dstintf",),
        "sourceAddresses": ("srcaddr",),
        "destinationAddresses": ("dstaddr",),
        "services": ("service",),
        "schedule": ("schedule",),
        "action": ("action",),
        "nat": ("nat",),
        ...
        "ipsSensor": ("ips-sensor", "ips_sensor"),
        "comments": ("comments",),
    },
    FortiGateEndpoint.IPSEC_PHASE1: {
        "name": ("name", "_mkey"),
        "interface": ("interface",),
        "ikeVersion": ("ike-version", "ike_version"),
        "type": ("type",),
        "peerType": ("peertype", "peer-type"),
        "remoteGateway": ("remote-gw", "remote_gw"),
        "proposal": ("proposal",),
        "dhGroups": ("dhgrp",),
        "natTraversal": ("nattraversal", "nat-traversal"),
        "dpd": ("dpd",),
...
def project_response(
    ...
    *,
    ...
    if spec.collection:
        ...
        output["items"] = [
            _project_record(endpoint, record, state) for record in selected
        ]
        output["returned"] = len(selected)
        output["available"] = len(records)
        output["truncated"] = len(records) > len(selected)
    else:
        if not isinstance(results, Mapping):

...
from .checkpoint_scripts import CHECKPOINT_SCRIPTS, CheckpointScriptId
from .config import HyperVTarget
...
class CheckpointExecutionError(RuntimeError):
    """A fixed checkpoint lifecycle script failed or returned invalid bounded JSON."""


class CheckpointPowerShellRunner:
    """Run only repository-owned checkpoint scripts through a dedicated JEA endpoint."""

    def __init__(self, executable: str):
        executable = executable.strip()
        if not executable:
            raise ValueError("HYPERV_POWERSHELL_EXECUTABLE must not be blank")
        basename = executable.replace("\\", "/").rsplit("/", 1)[-1].casefold()
        if basename not in _ALLOWED_EXECUTABLES:
            raise ValueError("HYPERV_POWERSHELL_EXECUTABLE must resolve to PowerShell or pwsh")
...
if ([string]::IsNullOrWhiteSpace($target) -or [string]::IsNullOrWhiteSpace($configuration)) {{ throw 'Checkpoint target configuration is incomplete' }}
$remoteProbe = {{
...
if ($output.Count -ne 1) {{ throw 'Remote checkpoint operation returned an unexpected output shape' }}
[Console]::Out.Write([string]$output[0])
""".strip()

    def run(
        self,
        script_id: CheckpointScriptId,
        target: HyperVTarget,
        ...
        *,
        ...
        if target.transport != "winrm" or target.configuration_name is None:
            raise PermissionError("checkpoint lifecycle requires a dedicated WinRM/JEA target")
        try:
            script = CHECKPOINT_SCRIPTS[script_id]
        except KeyError as exc:
            raise ValueError(f"Unknown checkpoint script id: {script_id}") from exc
        if timeout_seconds <= 0:
            raise ValueError("timeout_seconds must be positive")
        if not 1_024 <= max_response_bytes <= 1_048_576:
            raise ValueError("checkpoint response byte limit must be between 1024 and 1048576")

        payload_json = json.dumps(dict(payload), ensure_ascii=False, separators=(",", ":"))
        if len(payload_json.encode()) > _MAX_INPUT_BYTES:
            raise ValueError("checkpoint payload exceeds the fixed input byte limit")
            ...
            with tempfile.TemporaryFile(mode="w+b") as stdout_file, tempfile.TemporaryFile(
                ...
                if stdout_size > max_response_bytes:
                    raise CheckpointExecutionError(
                        "checkpoint operation exceeded the configured response byte limit"
                    )
                stdout_file.seek(0)
                ...
                if completed.returncode != 0:
                    ...
                    raise CheckpointExecutionError(
                        f"checkpoint operation {script_id.value} failed with exit code {completed.returncode}{suffix}"
                    )
        except subprocess.TimeoutExpired as exc:
            raise CheckpointExecutionError(
                f"checkpoint operation {script_id.value} timed out after {timeout_seconds:.1f}s"
            ) from exc
        ...
        if not raw:
            raise CheckpointExecutionError(
                f"checkpoint operation {script_id.value} returned no JSON output"
            )
        try:
            result = json.loads(raw.decode("utf-8").strip())
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise CheckpointExecutionError(
                f"checkpoint operation {script_id.value} returned invalid UTF-8 JSON"
            ) from exc
        if not isinstance(result, dict):
            raise CheckpointExecutionError(
                f"checkpoint operation {script_id.value} must return a JSON object"
            )

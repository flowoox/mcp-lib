...
class PowerShellExecutionError(RuntimeError):
    """A static Hyper-V diagnostic probe failed or returned invalid bounded JSON."""


class PowerShellRunner:
    """Run repository-owned read-only Hyper-V probes locally or through constrained JEA."""
...
if ($output.Count -ne 1) {{ throw 'Remote Hyper-V probe returned an unexpected output shape' }}
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            raise ValueError(f"Unknown Hyper-V script id: {script_id}") from exc
        ...
        if len(payload_json.encode("utf-8")) > _MAX_INPUT_BYTES:
            raise ValueError("Hyper-V diagnostic payload exceeds the fixed input byte limit")
            ...
            with tempfile.TemporaryFile(mode="w+b") as stdout_file, tempfile.TemporaryFile(
                ...
                if stdout_size > max_response_bytes:
                    raise PowerShellExecutionError(
                        "Hyper-V diagnostic probe exceeded the configured response byte limit"
                    )
                stdout_file.seek(0)
                ...
                if completed.returncode != 0:
                    ...
                    raise PowerShellExecutionError(
                        f"Hyper-V probe {script_id.value} failed with exit code {completed.returncode}{suffix}"
                    )
        except subprocess.TimeoutExpired as exc:
            raise PowerShellExecutionError(
                f"Hyper-V probe {script_id.value} timed out after {timeout_seconds:.1f}s"
            ) from exc
        ...
        if not raw:
            raise PowerShellExecutionError(
                f"Hyper-V probe {script_id.value} returned no JSON output"
            )
        try:
            ...
            raise PowerShellExecutionError(
                f"Hyper-V probe {script_id.value} returned invalid UTF-8 JSON"
            ) from exc
        if not isinstance(result, dict):
            raise PowerShellExecutionError(
                f"Hyper-V probe {script_id.value} must return a JSON object"
            )
